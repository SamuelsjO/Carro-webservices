...
public class JwtUtil {
	...
	//Chave com algoritmo de HS512
	// https://www.allkeysgenerator.com/
	private static final String JWT_SECRET = "q3t6w9z$C&F)J@NcRfUjXn2r4u7x!A%D*G-KaPdSgVkYp3s6v8y/B?E(H+MbQeTh";
	...
	public static List<GrantedAuthority> getRoles(String token){
		...
		return ((List<?>) claims
				.get("rol")).stream().map(authority -> new SimpleGrantedAuthority((String) authority)).collect(Collectors.toList());
	}
	...
	public static boolean isTokenValid(String token) {
		...
		if(nonNull(claims)) {
			...
			return 
					login != null && expiration != null && now.before(expiration);
		}
		return false;
	}
